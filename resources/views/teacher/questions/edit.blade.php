...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between border-b border-slate-200 pb-3">
        <div>
            <h2 class="text-xl font-bold text-slate-900 tracking-tight">Edit Soal Pilihan Ganda</h2>
    ...
    <form action="{{ route('guru.exams.questions.update', [$exam->id, $question->id]) }}" method="POST" class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="question_text" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Pertanyaan / Soal *</label>
            <textarea id="question_text" name="question_text" rows="4" required class="w-full text-xs p-3 border border-slate-300 rounded focus:ring-blue-500">{{ old('question_text', $question->question_text) }}</textarea>
        </div>
        ...
        <div>
            ...
            <div class="space-y-3">
                @foreach(['A', 'B', 'C', 'D', 'E'] as $key)
                    @php $opt = $options->get($key); @endphp
                    <div class="flex items-center space-x-3">
                        ...
                        <input type="text" name="options[{{ $key }}]" value="{{ old('options.'.$key, $opt ? $opt->option_text : '') }}" required
        ...
        @php
            $currentCorrect = $question->correctOption ? $question->correctOption->option_key : 'A';
        @endphp
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3 border-t border-slate-200">
            <div>
                ...
                <select id="correct_option" name="correct_option" required class="w-full text-xs p-2.5 border border-slate-300 rounded font-semibold text-emerald-800 bg-emerald-50">
                    @foreach(['A', 'B', 'C', 'D', 'E'] as $key)
                        <option value="{{ $key }}" {{ old('correct_option', $currentCorrect) === $key ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input type="number" step="0.5" id="points" name="points" value="{{ old('points', $question->points) }}" min="0.1" required
